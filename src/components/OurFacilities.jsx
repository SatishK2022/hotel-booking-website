...
function OurFacilities() {
  return (
    <Container className="py-20">
      <div className="h-[20rem] grid gap-5 grid-cols-3">
        ...
        <div className="grid grid-cols-4 gap-5 mt-0 py-0 col-span-2">
          {ourFacilities &&
            ourFacilities.map((card) => (
              <div
                key={card.name}
                className="flex flex-col gap-2 items-center justify-center border-2 border-slate-200 hover:border-green-600 rounded-md hover:shadow-lg duration-500"
              >
                <p>{card.icon}</p>
